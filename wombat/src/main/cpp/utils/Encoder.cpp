...
utils::Encoder::Encoder(double encoderTicksPerRotation, double reduction, int type) : _encoderTicksPerRotation(encoderTicksPerRotation), _reduction(reduction), _type(type) {};
...
units::radians_per_second_t utils::Encoder::GetEncoderAngularVelocity() {
  // return Getutils::EncoderTickVelocity() / (double)GetEncoderTicksPerRotation() * 2 * 3.1415926;
...
utils::TalonSRXEncoder::TalonSRXEncoder(ctre::phoenix::motorcontrol::can::TalonSRX *controller, double ticksPerRotation, double reduction) 
...
utils::CanEncoder::CanEncoder(int deviceNumber, double ticksPerRotation, double reduction)
