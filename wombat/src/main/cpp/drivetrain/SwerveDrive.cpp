    ...
    case SwerveModuleState::kPID: {
      auto feedforward = _config.driveMotor.motor.Voltage(
          0_Nm, units::radians_per_second_t{
                    (_velocityPIDController.GetSetpoint() / _config.wheelRadius)
                        .value()});
      driveVoltage =
          _velocityPIDController.Calculate(GetSpeed(), dt, feedforward);
      turnVoltage = _anglePIDController.Calculate(
          _config.turnMotor.encoder->GetEncoderPosition(), dt);
    } break;
  }
...
void SwerveModule::SetPID(units::radian_t angle,
  ...
  _state = SwerveModuleState::kPID;
  // @liam start added
  double diff = std::fmod((_anglePIDController.GetSetpoint() - angle)
                              .convert<units::degree>()
                              .value(),
                          360);
  if (std::abs(diff) >= 90) {
...
units::meters_per_second_t SwerveModule::GetSpeed() const {
  return units::meters_per_second_t{
      _config.driveMotor.encoder->GetEncoderAngularVelocity().value() *
  ...
  switch (_state) {
    ...
    case SwerveDriveState::kPose: {
      ...
      _target_fr_speeds.omega =
          _anglePIDController.Calculate(GetPose().Rotation().Radians(), dt);
    }
      [[fallthrough]];
...
bool SwerveDrive::IsAtSetPose() {
  return _anglePIDController.IsStable() && _xPIDController.IsStable() &&
