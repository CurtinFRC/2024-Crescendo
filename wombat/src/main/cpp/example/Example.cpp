...
// the initializer takes a variable which should then have brackets added to the
// end of it as if it was a function and then assigns that variable the value or
// the parameter placed inside the brackets we see this done here with the most
// common usage assigning _config the subsystem's configuration you can also add
...
// doing here switching between states based on _state and performing certain
// logic based on what state we are in. in kIdle we do nothing the point of the
// kIdle state. in kRunning we use a ternary operator to change the speed of our
// motor based on the y value of the left joystick on our controller
