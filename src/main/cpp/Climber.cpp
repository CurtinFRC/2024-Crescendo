...
void Climber::OnUpdate(units::second_t dt) {
  switch (_state) {
    ...
    case ClimberState::kArmUp: 
    {
      ...
      _table->GetEntry("PID OUTPUT").SetDouble(_pid.Calculate((-_config.climberGearbox.encoder->GetEncoderPosition().value()) * 100));
    } 
    break;
  ...
  _table->GetEntry("Encoder Output: ").SetDouble(_config.climberGearbox.encoder->GetEncoderPosition().value() * 100);
}
