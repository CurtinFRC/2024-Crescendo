...
std::pair<units::meter_t, units::degree_t> Vision::GetDistanceToTarget(VisionTarget target) {
    ...
    for (int i = 0; i < tags.size(); i++) {
        if (tags[i].id == (int)target) {         

            frc::Pose3d aprilTagPos = frc::Pose3d(tags[i].transform[0][3] * 1_m, tags[i].transform[1][3] * 1_m, tags[i].transform[2][3] * 1_m, frc::Rotation3d(0_deg, 0_deg, 0_deg));   
            
            units::meter_t distance = pose.Translation().Distance(aprilTagPos.Translation());

            units::degree_t angle = units::math::atan((pose.X() - units::meter_t{tags[i].transform[0][3]}) / (pose.Y() - units::meter_t{tags[i].transform[1][3]}));
...
std::pair<units::meter_t, units::degree_t> Vision::GetDistanceToTarget(int id) {
    ...
    for (int i = 0; i < tags.size(); i++) {
        if (tags[i].id == id) {
            ...
            units::degree_t angle = units::math::atan((currentPose.X() - units::meter_t{tags[i].transform[0][3]}) / (currentPose.Y() - units::meter_t{tags[i].transform[1][3]}));
